...
def verify_password(plain_password: str, hashed_password: str) -> bool:
    try:
        ...
        logger.error(f"Erro ao verificar com pwd_context: {str(e)}")
        try:
            # Usar o bcrypt diretamente como fallback
            result = bcrypt.checkpw(plain_password.encode('utf-8'), hashed_password.encode('utf-8'))
            logger.info(f"Resultado da verificação com bcrypt direto: {result}")
            return result
        except Exception as e2:
            logger.error(f"Erro ao verificar com bcrypt direto: {str(e2)}")
            # Se o hash for o do test_db.py (hard-coded), vamos fazer uma verificação manual
